import { luhnCheckDigit, luhnIsValid, luhnTrace } from "./luhn.js";
import { expectedBodyLength, matchingSchemes, schemeForId, suggestedLengthsForPrefix } from "./schemes.js";
...
function renderTrace(trace) {
  ...
  trace.steps.forEach((row, idx) => {
    ...
    if (row.doubled) tr.classList.add("trace-row-doubled");
    tr.innerHTML = `
      <td>${idx + 1}</td>
...
function compute(digits, scheme) {
  const check = luhnCheckDigit(digits);
  const full = digits + String(check);
  const schemes = matchingSchemes(digits);
  const schemeIds = schemes.map((s) => s.id);
  const lengthsHint = suggestedLengthsForPrefix(digits);

  let lengthWarning = null;
  if (scheme) {
    const exp = expectedBodyLength(scheme);
    if (exp != null && digits.length !== exp) {
      const sch = schemeForId(scheme);
      const label = sch ? sch.label : scheme;
      lengthWarning = {
        message: `${label} uses ${exp + 1} digits total; you entered ${digits.length} before the check digit.`,
      };
    }
  }

  const trace = luhnTrace(digits);
  return {
    body: digits,
    check_digit: check,
    full_number: full,
    valid_if_completed: luhnIsValid(full),
    matching_scheme_ids: schemeIds,
    suggested_total_lengths: lengthsHint,
...
function refresh() {
  ...
  try {
    const data = compute(digits, scheme);
    hideBanner();
    if (data.length_warning) showBanner(data.length_warning.message);
    ...
    showBanner(e?.message || "Something went wrong.");
    resultsEl.hidden = true;
    liveResults.textContent = "";
    traceTable.hidden = true;
    traceBody.innerHTML = "";
